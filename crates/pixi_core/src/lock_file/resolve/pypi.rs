...
async fn lock_no_deps_registry_package(
    ...
    let pinned_version = match &spec.source {
        PixiPypiSource::Registry { version, .. } => match version {
            VersionOrStar::Version(specs) if specs.len() == 1 => {
                if !matches!(specs[0].operator(), pep440_rs::Operator::Equal) {
                    return Err(miette::miette!(
                        "no-deps registry requirement must be pinned with '=='"
                    ));
                }
                specs[0].version().clone()
            }
            _ => {
                return Err(miette::miette!(
                    "no-deps registry requirement must be pinned with '=='"
    ...
    let pinned_uv = to_uv_version(&pinned_version).into_diagnostic()?;
        ...
        match format {
            MetadataFormat::Simple(simple) => {
                ...
                for datum in simple.iter() {
                    ...
                    let (file, requires_python) = if let Some((file, _)) = best_wheel {
                        (
                            file,
                            datum
                                .metadata
                    ...
                    let location = get_url_or_path(index_url, &file.url, abs_project_root)
                        .into_diagnostic()
                        .context("cannot convert registry dist")?;
                    let hash = parse_hashes_from_hash_vec(&file.hashes)
                        .into_diagnostic()
                        .context("cannot parse hashes for registry dist")?;
            ...
            MetadataFormat::Flat(entries) => {
                ...
                let mut best_sdist: Option<(&uv_distribution_types::File, &IndexUrl)> = None;

                for entry in &entries {
                    match &entry.filename {
                        DistFilename::WheelFilename(filename) => {
                            if filename.version != pinned_uv {
                                continue;
                            }
                            ...
                            let wheel_priority = (priority, filename.build_tag());
                            match best_wheel {
                                None => {
                                    best_wheel = Some((&entry.file, &entry.index, wheel_priority));
                                }
                                Some((_, _, best_priority)) => {
                                    if wheel_priority > best_priority {
                                        best_wheel =
                                            Some((&entry.file, &entry.index, wheel_priority));
                                    }
                                }
                            }
                        }
                        DistFilename::SourceDistFilename(filename) => {
                            if filename.version == pinned_uv && best_sdist.is_none() {
                                best_sdist = Some((&entry.file, &entry.index));
                            }
                        }
                    }
                }

                let (file, index_url) = if let Some((file, index, _)) = best_wheel {
                    (file, index)
                } else if let Some((file, index)) = best_sdist {
                    (file, index)
                } else {
                    continue;
                };
                ...
                let location = get_url_or_path(index_url, &file.url, abs_project_root)
                    .into_diagnostic()
                    .context("cannot convert flat index dist")?;
                let hash = parse_hashes_from_hash_vec(&file.hashes)
                    .into_diagnostic()
                    .context("cannot parse hashes for flat index dist")?;
    ...
    Err(miette::miette!(
        "failed to lock no-deps package '{name}' at version '{pinned_version}'",
        name = name,
